...
const buildConfigTemplatePath = path.join(__dirname, "build-transform-iw.json");
const buildConfigTemplateDir = path.dirname(buildConfigTemplatePath);
const cacheRoot = path.join(os.tmpdir(), "ironwall-json-transformer-cache");
const cachedSourcePath = path.join(cacheRoot, "transformer.c");
const cachedBinaryPath = path.join(cacheRoot, "transformer.out");
...
function exitWithChildStatus(error) {
  ...
  if (typeof error?.signal === "string") {
    process.stderr.write(`transformer terminated by signal ${error.signal}\n`);
    process.exit(1);
  }
  throw error;
}
...
function buildCachedTransformer() {
  fs.mkdirSync(cacheRoot, { recursive: true });
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "ironwall-json-transform-"));
...
function transformerBinaryPath() {
  if (process.env.IRONWALL_JSON_TRANSFORMER_BIN) {
    return process.env.IRONWALL_JSON_TRANSFORMER_BIN;
  }
  if (!cachedBinaryIsFresh()) {
    buildCachedTransformer();
  }
...
function main() {
  ...
  try {
    const invocation = buildWrappedChildInvocation(transformerBinaryPath(), repoRelativeInputPath);
    cp.execFileSync(invocation.command, invocation.args, {
      cwd: repoRoot,
      stdio: "inherit"
    });
