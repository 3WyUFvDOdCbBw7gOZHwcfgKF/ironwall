...
export function validateLinearizedProgram(program: LinearizedProgram): void {
    validateLinearBody(program.topLevelBody);
    program.functions.forEach((fn) => validateLinearBody(fn.body));
}

export function linearizePass(program: TypedSlotProgram | FoldedTypedPrimitiveProgram | ScalarReplacedFreshProgram): LinearizedProgram {
    ...
    validateLinearizedProgram(linearized);
    return linearized;
}
