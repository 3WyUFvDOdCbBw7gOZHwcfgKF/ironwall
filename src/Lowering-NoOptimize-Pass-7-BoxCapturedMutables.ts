// No-opt lowering pass 7 captured mutable boxing.
...
function collectProgramBoxingPlan(program: AnfProgram, analysis: FreeVarAnalysisResult): BoxingPlan {
    ...
    program.topLevelStatements.forEach((statement: LoweredExpr) => collectMutatedLocals(statement, new Set<string>()).forEach((name: string) => topLevelMutables.add(name)));
    program.topLevelStatements.forEach((statement: LoweredExpr) => {
        collectBoxRequirements(statement, new Set(), topLevelMutables, analysis, plan, state);
    });
    for (const fn of program.functions) {
        const fnScope = new Set<string>(fn.params.map((param: LoweredBinding) => param.name));
        ...
        const boxedParams = new Set<string>(fn.params.map((param: LoweredBinding) => param.name).filter((name: string) => fnRequirements.has(name)));
        if (boxedParams.size > 0) {
...
export function validateNoOptimizeCapturedMutableCheckProgram(program: CapturedMutableCheckedProgram): void {
    if (program.diagnostics.length !== 0) {
        throw new Error("Pass 7 mutable-capture validation failed: boxing pass should not emit diagnostics");
    }
}

export function noOptimizeBoxCapturedMutablesPass(program: AnfProgram, analysis: FreeVarAnalysisResult): CapturedMutableCheckedProgram {
    ...
    validateNoOptimizeCapturedMutableCheckProgram(result);
    return result;
}
