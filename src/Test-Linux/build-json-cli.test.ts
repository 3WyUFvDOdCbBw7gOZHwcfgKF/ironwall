...
const seqFixtureDir = join(repoRoot, "src", "Test-Linux", "Fixtures", "seq-var-scope");
const precompiledFixtureRoot = join(repoRoot, "src", "Test-Linux", "Fixtures", "precompiled-lib");
...
function assertRunConfigResult(configPath: string, expectedExitCode: number, label: string): void {
    ...
    strictEqual(result.status, expectedExitCode & 0xff, `${label} exit code mismatch\nstdout:\n${result.stdout}\nstderr:\n${result.stderr}`);
...
function compileStaticArchive(archivePath: string, sourceDir: string, sources: readonly FfiSourceCase[]): void {
    ...
    sources.forEach((source, index) => {
        ...
        execFileSync("cc", ["-O0", "-std=c11", "-c", cPath, "-o", objectPath], {
            cwd: repoRoot,
            encoding: "utf8",
            maxBuffer: MAX_BUFFER_BYTES,
            timeout: TEST_TIMEOUT_MS
    ...
    execFileSync("ar", ["rcs", archivePath, ...objectPaths], {
        cwd: repoRoot,
        encoding: "utf8",
        maxBuffer: MAX_BUFFER_BYTES,
        timeout: TEST_TIMEOUT_MS
