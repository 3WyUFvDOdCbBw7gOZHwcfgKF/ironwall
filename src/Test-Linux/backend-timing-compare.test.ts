...
const backendTimingFixtureDir: string = join(repoRoot, "src", "Test-Linux", "Fixtures", "backend-timing");
...
function verifyBackendTimingWorkload(): void {
    ...
    for (const run of runs) {
        ...
        while (sampleIndex < TIMING_SAMPLE_COUNT) {
            ...
            strictEqual(measured.status, backendTimingExpectedExitCode, `${run.label} backend-timing exit code mismatch\nstdout:\n${measured.output}\nstderr:\n${measured.stderr}`);
