...
const fixtureDir = join(repoRoot, "src", "Test-Linux", "Fixtures", "gc-explicit-only");
...
strictEqual(result.signal, null, `gc explicit-only signal mismatch\nstdout:\n${result.stdout}\nstderr:\n${result.stderr}`);
strictEqual(result.status, 74, `gc explicit-only result mismatch\nstdout:\n${result.stdout}\nstderr:\n${result.stderr}`);
strictEqual(result.stderr, "", `gc explicit-only stderr mismatch\n${result.stderr}`);
...
strictEqual(firstChunk.filter((line) => line.startsWith("gc-heap ")).length, 17, `expected seventeen gc-heap lines in first chunk\n${output}`);
strictEqual(firstChunk.filter((line) => line.startsWith("gc-live-heap ")).length, 5, `expected five gc-live-heap lines in first chunk\n${output}`);
strictEqual(firstChunk.filter((line) => line === "gc-live-summary live_heap=5 dead_heap=12").length, 1, `unexpected first gc-live-summary line\n${output}`);
strictEqual(firstChunk.filter((line) => line === "gc-summary frames=2 heap=17").length, 1, `unexpected first gc-summary line\n${output}`);
strictEqual(firstChunk.filter((line) => line === "gc-sweep-summary reclaimed=12 remaining_heap=5").length, 1, `unexpected first gc-sweep-summary line\n${output}`);
...
ok(secondChunk.some((line) => line.includes('data="hhhhh"')), `missing live dynamic text payload after explicit gc_collect\n${output}`);
ok(secondChunk.some((line) => line.includes("heap:class:test~gc~explicit~only@Payload") && line.includes("size=imm(2)")), `missing live payload object after explicit gc_collect\n${output}`);

process.stdout.write("gc-explicit-only no-optimized-frontend c-backend ok\n");
