...
function lowerStatementList(expressions: readonly LoweredExpr[]): readonly LoweredExpr[] {
    ...
    if (head.kind === "dvar") {
        ...
        return [{
            ...
            bindings: [{
                bind: head.bind,
                value: desugarCoreExpr(head.value)
            }],
            body
        }];
    }
    return [desugarCoreExpr(head), ...lowerStatementList(tail)];
}
...
function desugarCondToIf(expr: Extract<LoweredExpr, { readonly kind: "cond" }>): LoweredExpr {
    ...
    if (expr.clauses.length === 1) {
        ...
        return desugarCoreExpr(clause.body);
    }
    const [head, ...tail] = expr.clauses;
    return {
        kind: "if",
        condExpr: desugarCoreExpr(head.cond),
        trueBranchExpr: desugarCoreExpr(head.body),
        falseBranchExpr: desugarCondToIf({ kind: "cond", clauses: tail })
    };
}

export function desugarCoreExpr(expr: LoweredExpr): LoweredExpr {
    switch (expr.kind) {
        case "identifier":
        case "number_literal":
        case "text_literal":
        ...
        case "fn":
            return {
                ...expr,
                body: desugarCoreExpr(expr.body)
            };
        case "let":
            return {
                ...expr,
                bindings: expr.bindings.map((binding) => ({
                    ...binding,
                    value: desugarCoreExpr(binding.value)
                })),
                body: desugarCoreExpr(expr.body)
            };
        case "if":
            return {
                ...expr,
                condExpr: desugarCoreExpr(expr.condExpr),
                trueBranchExpr: desugarCoreExpr(expr.trueBranchExpr),
                falseBranchExpr: desugarCoreExpr(expr.falseBranchExpr)
            };
        case "while":
            return {
                ...expr,
                condExpr: desugarCoreExpr(expr.condExpr),
                bodyExpr: desugarCoreExpr(expr.bodyExpr)
            };
        case "cond":
        ...
        case "dvar":
            return {
                ...
                bindings: [{
                    bind: expr.bind,
                    value: desugarCoreExpr(expr.value)
                }],
                body: {
                    kind: "identifier",
                    name: expr.bind.name
        ...
        case "call":
            return {
                ...expr,
                callee: desugarCoreExpr(expr.callee),
                args: expr.args.map((arg) => desugarCoreExpr(arg))
            };
        case "direct_call":
            return {
                ...expr,
        ...
        case "object_get_field":
            return {
                ...expr,
                receiver: desugarCoreExpr(expr.receiver)
            };
        ...
        case "match":
            return {
                ...expr,
                unionExpr: desugarCoreExpr(expr.unionExpr),
                branches: expr.branches.map((branch) => ({
                    ...branch,
                    body: desugarCoreExpr(branch.body)
                }))
            };
    }
}
...
export function validateDesugaredCoreProgram(program: DesugaredCoreProgram): void {
...
export function desugarCorePass(program: LiftedLoweringProgram): DesugaredCoreProgram {
