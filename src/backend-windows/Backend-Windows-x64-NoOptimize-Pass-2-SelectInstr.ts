// Minimal no-opt x64 backend pass 2: select instructions.
import type {
    X64CallConvention,
    X64MirBody,
    X64MirInstruction,
...
const WIN64_GPR_ARG_REGS: readonly X64PhysicalRegisterName[] = ["rcx", "rdx", "r8", "r9"];
const WIN64_XMM_ARG_REGS: readonly X64PhysicalRegisterName[] = ["xmm0", "xmm1", "xmm2", "xmm3"];
...
function stackArg(index: number, bank: X64MirRegisterBank): X64SelectedOperand {
    return { kind: "stack_arg", index, bank };
}
...
function appendCallArgMoves(
    ...
    const gprArgRegs = callingConvention === "win64" ? WIN64_GPR_ARG_REGS : SYSV_GPR_ARG_REGS;
    const xmmArgRegs = callingConvention === "win64" ? WIN64_XMM_ARG_REGS : SYSV_XMM_ARG_REGS;
    ...
    for (const arg of args) {
        ...
        if (bank === "xmm") {
            ...
            if (callingConvention === "win64") {
                stackArgs.push(lowered);
                continue;
            }
            instructions.push({
                kind: "copy",
                target: stackArg(nextStack, "xmm"),
                source: lowered
            });
            nextStack += 1;
            continue;
...
function appendIncomingParamMoves(instructions: X64SelectedInstruction[], params: readonly X64MirVirtualRegisterOperand[]): void {
    ...
    for (const param of params) {
        if (param.bank === "xmm") {
            const registerName = SYSV_XMM_ARG_REGS[nextXmm];
            if (registerName) {
                instructions.push({
                    kind: "copy",
                    target: param,
        ...
        const registerName = SYSV_GPR_ARG_REGS[nextGpr];
        if (registerName) {
            instructions.push({
                kind: "copy",
                target: param,
...
function lowerHelperCall(
    ...
    callingConvention: X64CallConvention = "win64"
