...
function implicitBarrierReadKeysForCall(copies: readonly Extract<X64RegAllocatedInstruction, { kind: "copy" }>[]): ReadonlySet<string> {
    const keys = new Set<string>();
    for (const copy of copies) {
        switch (copy.target.kind) {
            case "preg":
                if (SYSV_GPR_ARG_REGS.has(copy.target.name) || SYSV_XMM_ARG_REGS.has(copy.target.name)) {
    ...
    if ("kind" in barrier) {
        switch (barrier.kind) {
            case "copy":
                ...
                break;
            case "call_direct":
                barrier.stackArgs.forEach(addOperand);
                for (const key of implicitBarrierReadKeysForCall(copies)) {
                    keys.add(key);
                }
                break;
            case "call_indirect":
