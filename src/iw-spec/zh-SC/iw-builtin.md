...
- frontend surface sugar 另外接受 `add` / `sub` / `mul` / `and` / `or` 的 `>= 2` 参数写法，语义上收敛成右结合 binary tree；例如 `(add a b c d)` 等价于 `(add a (add b (add c d)))`
...
- `print`、`sin`、`val_to_f7`、`bin_to_f7` 这类 `std~...` 导出名只是在对应 package 内保留的普通 export，不是语言 builtin
