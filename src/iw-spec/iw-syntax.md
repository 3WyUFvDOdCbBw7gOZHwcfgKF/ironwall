...
語言 builtin 名字與 package 導出名字由 builtin 與模組規格分別定義，不在本節重複列出。
...
`public` 只能出現在普通 class 或 generic class 的 class body 內，且只能包住單一 property 或 method。未包 `public` 的 property / method 默認為 private；constructor 默認 public，不能寫成 `(public (constructor ...))`。
...
- `add` / `mul` / `and` / `or` 可寫成 `>= 2` 參數；parser 會把它們收斂成右結合 binary tree
- `sub` 也可寫成 `>= 2` 參數，但 parser 會把它收斂成左結合 binary tree
...
- `export` 只能出現在 top-level。
- `(export exp)` 必須且只能有一個參數。
- `export` 只能包住 top-level `class`、泛型 `class`、`function`、`declare`、泛型 `function` 或 top-level `var`。
- `export` 不改變被包裹定義的型別與求值語義，只控制跨 package 可見性。
