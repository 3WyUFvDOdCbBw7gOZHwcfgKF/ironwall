// Minimal no-opt x64 backend pass 4: frame layout.
import type {
    X64FrameLayoutBody,
...
function outgoingStackArgBank(operand: X64RegAllocatedOperand): X64MirRegisterBank {
    switch (operand.kind) {
        case "preg":
        case "stack_slot":
        case "stack_arg":
...
function layoutBody(body: X64RegAllocatedBody): X64FrameLayoutBody {
    ...
    for (const block of body.blocks) {
        for (const instruction of block.instructions) {
            ...
            instruction.stackArgs.forEach((operand, index) => {
                if (!outgoingSlots.has(index)) {
                    outgoingSlots.set(index, {
                        kind: "outgoing_arg",
                        index,
                        bank: outgoingStackArgBank(operand),
                        offsetFromRbp: 0,
                        size: SLOT_SIZE_BYTES
                    });
                }
