// Minimal no-opt x64 backend pass 6: emit textual assembly.
import type {
    X64CallConvention,
    X64FrameLayoutBody,
    X64FrameSlotLayout,
...
import { buildX64TextualAssembly } from "./Backend-Linux-x64-TextualAssembly";
import { x64NativeGcFrameInitSymbol, x64NativeGcFrameKey, x64NativeTextValueSymbol } from "./Backend-Linux-X64-NativeSupport";

interface X64EmittableFunctionDefinition {
    readonly symbol: string;
    readonly body: X64FrameLayoutBody;
}

interface X64EmittableProgram {
    ...
    switch (operand.kind) {
        ...
        case "symbol": {
            return operand.symbol.startsWith("__iw_x64_direct_value_")
                ...
                : mangleAsmSymbol(operand.symbol);
        }
        case "text": {
...
function emitCopyLines(target: X64RegAllocatedOperand, source: X64RegAllocatedOperand, slots: ReadonlyMap<string, X64FrameSlotLayout>): readonly string[] {
    ...
    if (bank === "gpr" && targetIsMemory && (source.kind === "imm_i64" || isImmediateSymbolOperand(source) || source.kind === "text")) {
...
function callStackArgBank(operand: X64RegAllocatedOperand): X64MirRegisterBank {
    switch (operand.kind) {
        case "preg":
        case "stack_slot":
        case "stack_arg":
...
function emitSysvCallLines(
    ...
    stackArgs.forEach((operand, index) => {
        lines.push(...emitStoreToCallStackLines(
            outgoingArgAddress(callStackArgBank(operand), index, slots),
            operand,
            slots
        ));
    });
...
function emitCallLines(
    ...
    switch (callingConvention) {
        case "internal":
        case "sysv_c_ffi":
            return emitSysvCallLines(symbol, stackArgs, slots);
    }
}
...
function emitGcFrameBeginLines(
    ...
    const lines = [instruction.gcRoots.length > 0 ? asmComment(`gc_frame_begin ${instruction.gcRoots.join(", ")}`) : asmComment("gc_frame_begin <none>")];
    ...
    lines.push(`lea rdi, ${stackAddressFromRbpOffset(body.gcShadowFrameOffsetFromRbp)}`);
    lines.push(...emitCallLines(
        "sysv_c_ffi",
        x64NativeGcFrameInitSymbol(x64NativeGcFrameKey(instruction.gcRoots)),
        [],
        slots
    ));
    return lines;
    ...
    switch (instruction.kind) {
        ...
        case "call_direct": {
            return [
                ...
                ...emitCallLines(instruction.callingConvention, instruction.symbol, instruction.stackArgs, slots)
            ];
        }
        case "call_indirect": {
