...
import { PrimitiveTypeValue, getClassTypeId, getRuntimeTypeId, hashText, type TypeValue } from "./Backend-Linux-Typecheck-Core";
import {
...
import {
    performLinuxCBackendSourcePass1CollectSections,
    performLinuxCBackendX64SupportPass1CollectSections
} from "./Backend-Linux-C-Pass-1-CollectSections";
import {
    performLinuxCBackendSourcePass2AssembleRuntimeAndDriverSource,
    performLinuxCBackendX64SupportPass2AssembleSource
} from "./Backend-Linux-C-Pass-2-AssembleSource";
import { performLinuxCBackendSourcePass3SplitRuntimeAndDriverSource } from "./Backend-Linux-C-Pass-3-SplitSources";
import { buildLinuxCBuiltinArtifacts } from "./Backend-Linux-C-Builtins";
import {
    emitLinuxCDeclaredCHeapHostHelperRuntime,
    emitLinuxCDeclaredStdSysFfiRuntime,
    emitLinuxCExportedIwFunctionRuntime,
    generateLinuxCHeaderFromFinalBackendIR
} from "./Backend-Linux-C-HostInterop";
import { emitLinuxCGcCollectCoreRuntime } from "./Backend-Linux-C-GC-CollectCore";
import {
    emitLinuxCGcGlobalContentPrinter,
    emitLinuxCGcPrintPassRuntime
} from "./Backend-Linux-C-GC-PrintPass";
import type { LinuxCGcPrintPassDependencies } from "./Backend-Linux-C-GC-Shared";
import { emitLinuxCGcValidationPassRuntime } from "./Backend-Linux-C-GC-ValidationPass";
...
const LINUX_C_BUILTIN_ARTIFACTS = buildLinuxCBuiltinArtifacts({
...
function buildLinuxCHostInteropDependencies(
...
function generateCHeaderFromFinalBackendIR(program: FinalBackendIRProgram): string {
    return generateLinuxCHeaderFromFinalBackendIR(program);
}
...
function emitFunction(fn: BackendFunctionIR, functionNames: ReadonlySet<string>, program: ProgramCodegenArtifacts): string {
    ...
    return [
        `static inline ${cTypeForRepresentation(fn.resultRepresentation)} ${cFunctionName(fn.symbol)}(${params}) {`,
        localDeclarations,
        statements,
        resultDeclaration,
        result,
...
function emitPrototype(fn: BackendFunctionIR): string {
    const params = fn.params.map((param) => `${cTypeForRepresentation(fn.bindingRepresentations.get(param) ?? "reference")} ${cParamName(param)}`).join(", ");
    return `static inline ${cTypeForRepresentation(fn.resultRepresentation)} ${cFunctionName(fn.symbol)}(${params});`;
}
...
function resolveLinuxX64DirectCallConvention(
    symbol: string,
...
function emitX64DirectCallWrappers(program: FinalBackendIRProgram, codegen: ProgramCodegenArtifacts): string {
    ...
    for (const [symbol] of Array.from(codegen.functionArities.entries()).sort((left, right) => left[0].localeCompare(right[0]))) {
        if (resolveLinuxX64DirectCallConvention(symbol, programSymbols, externCallingConventions) === "internal") {
            continue;
        }
...
const LINUX_C_GC_PRINT_PASS_DEPENDENCIES: LinuxCGcPrintPassDependencies = {
    cFieldName,
    cGlobalName,
    cStringLiteral,
    cStructName,
...
function emitGcGlobalContentPrinter(
    ...
    return emitLinuxCGcGlobalContentPrinter(globalDescriptor, globalMetadata, LINUX_C_GC_PRINT_PASS_DEPENDENCIES);
}
...
function emitGcRuntimeSupport(
    ...
    return [
        emitLinuxCGcValidationPassRuntime(),
        emitLinuxCGcPrintPassRuntime(codegen, LINUX_C_GC_PRINT_PASS_DEPENDENCIES),
        emitLinuxCGcCollectCoreRuntime({
            globalInitLines,
...
export function generateCBackendSources(program: FinalBackendIRProgram, extraSupportSource = ""): CBackendSources {
    ...
    const hostInteropDeps = buildLinuxCHostInteropDependencies((rootNames, bodyLines, indentLevel) => emitStandaloneGcScopedBlock(rootNames, bodyLines, codegen, indentLevel));
    const stdSysFfiRuntime = emitLinuxCDeclaredStdSysFfiRuntime(program, hostInteropDeps);
    const declaredCHeapHostHelperRuntime = emitLinuxCDeclaredCHeapHostHelperRuntime(program);
    const exportedIwFunctionRuntime = emitLinuxCExportedIwFunctionRuntime(program, hostInteropDeps);
    ...
    const emittedFunctions = allFunctions
        ...
        .join("\n\n");
    const sourcePass1 = performLinuxCBackendSourcePass1CollectSections({
    ...
    const runtimeAndDriverSource = performLinuxCBackendSourcePass2AssembleRuntimeAndDriverSource(sourcePass1);
    const splitSources = performLinuxCBackendSourcePass3SplitRuntimeAndDriverSource(runtimeAndDriverSource, sourcePass1.driverResultType);
    return {
...
export function generateX64NativeSupportCFromFinalBackendIR(
    ...
    const sourcePass1 = performLinuxCBackendX64SupportPass1CollectSections({
        builtinHelpers: BUILTIN_HELPERS,
        classRuntime,
        gcFrameRuntime,
        runtimeDescriptors,
    ...
    return performLinuxCBackendX64SupportPass2AssembleSource(sourcePass1);
}
