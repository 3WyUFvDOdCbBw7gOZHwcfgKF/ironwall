...
import { GenericClassInstanceTypeValue, PrimitiveTypeValue, type TypeValue } from "./Backend-Linux-Typecheck-Core";
...
export interface LinuxCHostInteropDependencies {
...
function emitDeclaredStdSysFfiWrapper(rename: DeclaredStdSysFfiRename, deps: LinuxCHostInteropDependencies): string {
...
function emitExportedIwHostToValueExpression(type: TypeValue, expression: string, deps: LinuxCHostInteropDependencies): string {
    if (isPrimitiveTypeNamed(type, "i5")) {
...
function emitExportedIwValueToHostExpression(
    ...
    expression: string,
    usageContext: string,
    deps: LinuxCHostInteropDependencies
): string {
    if (isPrimitiveTypeNamed(type, "i5")) {
...
function emitExportedIwFunctionWrapper(artifact: ExportedIwFunctionArtifact, deps: LinuxCHostInteropDependencies): string {
...
export function emitLinuxCDeclaredStdSysFfiRuntime(program: FinalBackendIRProgram, deps: LinuxCHostInteropDependencies): string {
...
export function emitLinuxCDeclaredCHeapHostHelperRuntime(program: FinalBackendIRProgram): string {
...
export function emitLinuxCExportedIwFunctionRuntime(program: FinalBackendIRProgram, deps: LinuxCHostInteropDependencies): string {
...
export function generateLinuxCHeaderFromFinalBackendIR(program: FinalBackendIRProgram): string {
    const artifacts = collectExportedIwFunctionArtifacts(program);
    const requirements = collectExportedIwHostHelperRequirements(program);
    const body: string[] = [
