...
function collectOperandUses(operand: X64SelectedOperand, into: Map<string, X64MirRegisterBank>, position: number, positions: Map<string, { start: number; end: number }>): void {
...
function flattenBody(body: X64SelectedBody): { readonly positions: Map<string, { start: number; end: number }>; readonly banks: Map<string, X64MirRegisterBank>; readonly callPositions: readonly number[] } {
    ...
    for (const block of orderBlocks(body)) {
        ...
        for (const instruction of block.instructions) {
            ...
            switch (instruction.kind) {
                case "copy":
                    collectOperandUses(instruction.source, banks, position, positions);
                ...
                case "call_indirect":
                    callPositions.push(position);
                    collectOperandUses(instruction.callee, banks, position, positions);
                    break;
                case "gc_frame_begin":
                    callPositions.push(position);
                    instruction.gcRootOperands.forEach((operand) => collectOperandUses(operand, banks, position, positions));
                    break;
                case "gc_frame_end":
                    break;
                case "test":
                    collectOperandUses(instruction.left, banks, position, positions);
                    collectOperandUses(instruction.right, banks, position, positions);
                    break;
                ...
                case "pseudo_closure_create": {
                    ...
                    if (instruction.kind === "pseudo_object_get_field" || instruction.kind === "pseudo_slot_load") {
                        collectOperandUses(instruction.receiver, banks, position, positions);
                    } else if (instruction.kind === "pseudo_union_inject") {
                        collectOperandUses(instruction.value, banks, position, positions);
                    } else if (instruction.kind === "pseudo_union_has_tag" || instruction.kind === "pseudo_union_get_payload") {
                        collectOperandUses(instruction.unionValue, banks, position, positions);
                    } else if (instruction.kind === "pseudo_closure_create") {
                        instruction.captures.forEach((capture) => collectOperandUses(capture, banks, position, positions));
                    }
                    break;
                }
        ...
        switch (block.terminator.kind) {
            case "ret":
                break;
            case "jmp":
                block.terminator.args.forEach((arg) => collectOperandUses(arg, banks, position, positions));
                break;
            case "jcc":
                block.terminator.trueArgs.forEach((arg) => collectOperandUses(arg, banks, position, positions));
                block.terminator.falseArgs.forEach((arg) => collectOperandUses(arg, banks, position, positions));
                break;
        }
    }
    return { positions, banks, callPositions };
}

function buildIntervals(body: X64SelectedBody): readonly Interval[] {
    const { positions, banks, callPositions } = flattenBody(body);
...
export function linearScanRegallocX64Pass(program: X64CopyPropagatedProgram | X64SelectedProgram): X64RegAllocatedProgram {
    return {
