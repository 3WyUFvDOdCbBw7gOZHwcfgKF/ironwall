// No-opt lowering pass 3 method and constructor lifting.
...
import { collectNoOptimizeLoweringClassLayouts } from "./Lowering-NoOptimize-Pass-1-CollectLayouts";
import { createNoOptimizeLoweringSnapshotProgram } from "./Lowering-NoOptimize-Pass-0-Snapshot";
import type { NoOptimizeLoweringSnapshotOptions } from "./Lowering-NoOptimize-Pass-0-Snapshot";
import { lowerNoOptimizeClassPrimitivesPass } from "./Lowering-NoOptimize-Pass-2-LowerClassPrimitives";
...
export function validateNoOptimizeLiftedLoweringProgram(program: LiftedLoweringProgram, pass2: LoweredClassPrimitiveProgram): void {
...
export function liftNoOptimizeMethodsPass(program: LoweredClassPrimitiveProgram): LiftedLoweringProgram {
    ...
    validateNoOptimizeLiftedLoweringProgram(liftedProgram, program);
    return liftedProgram;
}

export function performNoOptimizeLoweringStageA(ast: LoweringSnapshotProgram, layouts?: LoweringLayoutTable): {
    readonly layouts: LoweringLayoutTable;
    readonly pass2: LoweredClassPrimitiveProgram;
    readonly pass3: LiftedLoweringProgram;
} {
    const resolvedLayouts = layouts ?? collectNoOptimizeLoweringClassLayouts(ast);
    const pass2 = lowerNoOptimizeClassPrimitivesPass(ast, resolvedLayouts);
    const pass3 = liftNoOptimizeMethodsPass(pass2);
    return {
...
export function performNoOptimizeLoweringStageAFromArtifacts(programAst: import("./AstNode").AstNode, artifacts: MonomorphizedArtifacts, options?: NoOptimizeLoweringSnapshotOptions): LoweringStageAResult {
    const snapshot = createNoOptimizeLoweringSnapshotProgram(programAst, artifacts, options);
    const layouts = collectNoOptimizeLoweringClassLayouts(snapshot);
    const pass2 = lowerNoOptimizeClassPrimitivesPass(snapshot, layouts);
    const pass3 = liftNoOptimizeMethodsPass(pass2);
    return {
        snapshot,
        layouts,
