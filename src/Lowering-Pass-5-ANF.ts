...
function bindAtomic(expr: LoweredExpr, context: AnfContext, cont: (atomicExpr: LoweredExpr) => LoweredExpr): LoweredExpr {
    const normalized = anfExpr(expr, context);
...
export function anfExpr(expr: LoweredExpr, context: AnfContext): LoweredExpr {
    switch (expr.kind) {
        case "identifier":
        case "number_literal":
        case "text_literal":
        ...
        case "fn":
            return {
                ...expr,
                body: anfExpr(expr.body, context)
            };
        case "let":
            return {
                ...expr,
                bindings: expr.bindings.map((binding) => ({
                    ...binding,
                    value: anfExpr(binding.value, context)
                })),
                body: anfExpr(expr.body, context)
            };
        case "if":
            return bindAtomic(expr.condExpr, context, (atomicCond) => ({
                kind: "if",
                condExpr: atomicCond,
                trueBranchExpr: anfExpr(expr.trueBranchExpr, context),
                falseBranchExpr: anfExpr(expr.falseBranchExpr, context)
            }));
        case "while":
            return {
                kind: "while",
                condExpr: anfExpr(expr.condExpr, context),
                bodyExpr: anfExpr(expr.bodyExpr, context)
            };
        case "seq":
            return {
                kind: "seq",
                expressions: expr.expressions.map((inner) => anfExpr(inner, context))
            };
        case "set_local":
            return {
                kind: "set_local",
                identifier: expr.identifier,
                value: anfExpr(expr.value, context)
            };
        case "call":
        ...
        case "match":
            return bindAtomic(expr.unionExpr, context, (atomicUnionExpr) => ({
                ...
                branches: expr.branches.map((branch) => ({
                    ...branch,
                    body: anfExpr(branch.body, context)
                }))
            }));
        case "cond":
        case "dvar":
...
export function anfPass(program: DesugaredCoreProgram): AnfProgram {
    const context: AnfContext = { tempCounter: 0 };
    const anfProgram: AnfProgram = {
        kind: "anf_program",
        topLevelStatements: program.topLevelStatements.map((statement) => anfExpr(statement, context)),
        globals: program.globals,
        functions: program.functions.map((fn): LoweredFunctionDefinition => ({
            ...fn,
            body: anfExpr(fn.body, context)
        })),
        declaredFunctions: program.declaredFunctions,
        layouts: program.layouts,
        metadata: program.metadata
    };
    validateAnfProgram(anfProgram);
    return anfProgram;
}

export function isAnfAtomicExpr(expr: LoweredExpr): boolean {
