...
import { buildX64TextualAssembly } from "./backend-linux/Backend-Linux-x64-TextualAssembly";
import { performLoweringStageCFromArtifacts } from "./Lowering-Pass-10-PackageBackendIR";
import { runPlatformReleaseCli, runPlatformReleaseCliMain, type PlatformReleaseHooks } from "./main-release-platform";

const linuxReleaseHooks: PlatformReleaseHooks = {
...
if (require.main === module) {
    runPlatformReleaseCliMain(process.argv.slice(2), linuxReleaseHooks);
}

export function runCli(argv: readonly string[]): number {
    return runPlatformReleaseCli(argv, linuxReleaseHooks);
}
