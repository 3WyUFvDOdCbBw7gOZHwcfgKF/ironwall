...
const suiteScriptPath: string = join(repoRoot, "build", "Test-Windows", "RunSuite.js");
const defaultSummaryPath: string = join(repoRoot, "artifacts", "windows-suite-summary.json");
...
function collectFailedTestFileName(lines: readonly string[]): string | null {
    for (const line of lines) {
        const match: RegExpMatchArray | null = line.match(/windows-suite failed in (.+)$/);
        if (match !== null) {
            return match[1];
        }
    }
...
const summary: SuiteSummary = {
    ...
    completionMarkerSeen: combinedLines.some((line) => /^windows-suite ok \d+ tests$/.test(line)),
