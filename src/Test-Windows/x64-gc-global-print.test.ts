...
const fixtureDir = join(repoRoot, "src", "Test-Windows", "Fixtures", "gc-global-print");
...
function runNative(supportC: string, assemblyText: string): string {
    const tempDir = mkdtempSync(join(tmpdir(), "ironwall-x64-gc-global-print-"));
    try {
...
for (const testCase of x64Cases) {
    ...
    ok(!supportC.includes("iw_gc_next_u64"), `${testCase.label} support C should not rely on runtime GC PRNG helpers`);
    ok(!supportC.includes("iw_gc_prng_state"), `${testCase.label} support C should not embed runtime GC PRNG state`);

    const output = runNative(supportC, asm);
    const lines = normalizeLines(output);
    strictEqual(lines[lines.length - 1], "16", `${testCase.label} gc global print result mismatch\n${output}`);
    ...
    ok(globalLines.some((line) => line.startsWith("gc-global global:test~gc~global~print@main addr=")), `${testCase.label} missing main-unit gc-global line\n${output}`);
    const mainLiveGlobalLine = liveGlobalLines.find((line) => line.startsWith("gc-live-global global:test~gc~global~print@main addr="));
    ok(mainLiveGlobalLine !== undefined, `${testCase.label} missing main-unit gc-live-global line\n${output}`);
    ok(mainLiveGlobalLine.includes("global_text=heap:builtin:text@") && mainLiveGlobalLine.includes('="hhhhh"'), `${testCase.label} missing global_text in live global\n${output}`);
    ok(mainLiveGlobalLine.includes("global_rank=imm(11)"), `${testCase.label} missing global_rank in live global\n${output}`);
    ok(
        mainLiveGlobalLine.indexOf("global_rank=") < mainLiveGlobalLine.indexOf("global_text="),
        `${testCase.label} expected canonical global field order\n${output}`
    );
    ...
    ok(
        mainLiveFrameLine.includes("heap:builtin:text@") && mainLiveFrameLine.includes('="hhhhh"'),
        `${testCase.label} missing live frame roots\n${output}`
    );

    strictEqual(heapLines.length, 2, `${testCase.label} expected two gc-heap lines\n${output}`);
    strictEqual(liveHeapLines.length, 2, `${testCase.label} expected two gc-live-heap lines\n${output}`);
    ...
    ok(liveHeapLines.some((line) => line.includes("heap:builtin:text") && line.includes('data="hhhhh"')), `${testCase.label} missing live global text heap line\n${output}`);
    ...
    strictEqual(Number(liveSummaryMatch[1]), 2, `${testCase.label} unexpected live heap count\n${output}`);
    strictEqual(Number(liveSummaryMatch[2]), 0, `${testCase.label} unexpected dead heap count\n${output}`);

    process.stdout.write(`x64-gc-global-print ${testCase.label} ok\n`);
}
